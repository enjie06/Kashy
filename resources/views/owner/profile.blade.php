...
<main id="main" class="min-h-screen bg-kashy-cream">
  ...
  <div class="max-w-lg mx-auto lg:max-w-2xl px-4 pt-7 pb-20 space-y-5">
    ...
    {{-- Flash success/error --}}
    @if(session('success'))
      <div id="flashMsg" class="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-xl">
        {{ session('success') }}
      </div>
    @endif
    @if(session('error'))
      <div id="flashMsg" class="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3 rounded-xl">
        {{ session('error') }}
      </div>
    @endif
    ...
    <div class="anim-0 flex flex-col items-center text-center">
      <div class="avatar-anim relative mb-4">
          ...
          <div class="w-24 h-24 sm:w-28 sm:h-28 rounded-full overflow-hidden bg-stone-200">
            ...
            @else
              <div class="w-full h-full flex items-center justify-center text-white text-4xl font-bold"
                   style="background:linear-gradient(135deg,#c4a882,#9d7e5e,#b8966c);">
        ...
        <!-- Tombol upload foto -->
        <label for="photoInput" class="absolute bottom-0.5 right-0.5 w-7 h-7 rounded-full bg-terra shadow-md border-2 border-white flex items-center justify-center hover:bg-terra-l transition-colors cursor-pointer">
        ...
        <button onclick="confirmDeletePhoto()" type="button"
                class="absolute bottom-0.5 left-0.5 w-7 h-7 rounded-full bg-red-500 shadow-md border-2 border-white flex items-center justify-center hover:bg-red-600 transition-colors">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/></svg>
        </button>
        @endif
        <form id="photoForm" action="{{ route('profile.updatePhoto') }}" method="POST" enctype="multipart/form-data">
          @csrf
          <input type="file" id="photoInput" name="profile_photo" accept="image/*" class="hidden"
      ...
      <h1 class="font-display text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
        {{ Auth::user()->name }}
      </h1>
      <span class="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-semibold bg-terra-xs text-terra border border-terra-ll">
        <span class="w-1.5 h-1.5 rounded-full bg-terra"></span>
    ...
    <div class="anim-2 bg-white rounded-2xl shadow-sm border border-border overflow-hidden">
      ...
      <div class="px-5 pt-4 pb-1 flex items-center justify-between">
        ...
        <a href="{{ route('manajemen.toko') }}" class="text-[11px] font-semibold text-terra hover:underline">Edit</a>
      </div>
      <!-- Nama Toko -->
      <div class="info-row flex items-center gap-4 px-5 py-4 border-b border-stone-100">
        <div class="w-9 h-9 rounded-xl bg-terra-xs flex items-center justify-center flex-shrink-0">
        ...
        <div class="flex-1">
          ...
          <p class="text-sm font-medium text-gray-900">{{ $store->nama_toko ?? '-' }}</p>
        </div>
      </div>
      <!-- Alamat -->
      <div class="info-row flex items-center gap-4 px-5 py-4 border-b border-stone-100">
        ...
        <div class="flex-1">
          ...
          <p class="text-sm font-medium text-gray-900">
            {{ $store ? trim(($store->jalan ?? '') . ', ' . ($store->kota ?? '') . ', ' . ($store->negara ?? ''), ', ') : '-' }}
          </p>
        </div>
      </div>
      <!-- Instagram -->
      <a href="https://www.instagram.com/snd_store___?igsh=cmloaGdna3ptbnp5" target="_blank">
        <div class="info-row flex items-center gap-4 px-5 py-4 border-b border-stone-100">
          <div class="w-9 h-9 rounded-xl bg-terra-xs flex items-center justify-center flex-shrink-0">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C8966C" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="#C8966C"/></svg>
          </div>
          <div class="flex-1">
    ...
    <div class="anim-3 bg-white rounded-2xl shadow-sm border border-border overflow-hidden">
      <div class="px-5 pt-5 pb-6">
        ...
        <form id="profileForm" action="{{ route('profile.updateProfile') }}" method="POST">
          @csrf
          @method('PUT')
          <div class="space-y-4">
            <div>
              ...
              <input type="text" name="name" value="{{ old('name', Auth::user()->name) }}"
                     class="kashy-input w-full px-4 py-3 border border-border rounded-xl bg-bg text-sm text-gray-900"/>
              @error('name')<p class="field-error show">{{ $message }}</p>@enderror
            </div>
            <div>
              <label class="block text-[10px] font-bold tracking-wide text-muted mb-2">Email</label>
              <input type="email" name="email" value="{{ old('email', Auth::user()->email) }}"
                     class="kashy-input w-full px-4 py-3 border border-border rounded-xl bg-bg text-sm"/>
              @error('email')<p class="field-error show">{{ $message }}</p>@enderror
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="tel" name="phone" value="{{ old('phone', Auth::user()->phone) }}"
                       class="kashy-input w-full px-4 py-3 border border-border rounded-xl bg-bg text-sm"/>
                @error('phone')<p class="field-error show">{{ $message }}</p>@enderror
              </div>
              <div>
                <label class="block text-[10px] font-bold tracking-wide text-muted mb-2">Alamat</label>
                <input type="text" name="address" value="{{ old('address', Auth::user()->address) }}"
                       class="kashy-input w-full px-4 py-3 border border-border rounded-xl bg-bg text-sm"/>
                @error('address')<p class="field-error show">{{ $message }}</p>@enderror
              </div>
            </div>
            <button type="submit" class="btn-dark w-full py-3.5 text-white font-semibold rounded-xl text-sm tracking-wide flex items-center justify-center gap-2">
        ...
        <form id="passwordForm" action="{{ route('profile.updatePassword') }}" method="POST">
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="relative">
                <input type="password" name="current_password" id="pwLama"
                       class="kashy-input w-full px-4 py-3 pr-11 border border-border rounded-xl bg-bg text-sm"/>
                <button type="button" onclick="togglePw('pwLama','eyeL')" id="eyeL"
                        class="absolute right-3.5 top-1/2 -translate-y-1/2 text-stone-300 hover:text-terra transition-colors">
              ...
              @error('current_password')<p class="field-error show">{{ $message }}</p>@enderror
            </div>
            <div>
              <label class="block text-[10px] font-bold tracking-wide text-muted mb-2">Password Baru</label>
              <div class="relative">
                <input type="password" name="password" id="pwBaru"
                       class="kashy-input w-full px-4 py-3 pr-11 border border-border rounded-xl bg-bg text-sm"/>
                <button type="button" onclick="togglePw('pwBaru','eyeB')" id="eyeB"
                        class="absolute right-3.5 top-1/2 -translate-y-1/2 text-stone-300 hover:text-terra transition-colors">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>
                </button>
              </div>
            ...
            <div>
              ...
              <div class="relative">
                <input type="password" name="password_confirmation" id="pwKonfirm"
                       class="kashy-input w-full px-4 py-3 pr-11 border border-border rounded-xl bg-bg text-sm"/>
                <button type="button" onclick="togglePw('pwKonfirm','eyeK')" id="eyeK"
                        class="absolute right-3.5 top-1/2 -translate-y-1/2 text-stone-300 hover:text-terra transition-colors">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>
                </button>
              </div>
              @error('password')<p class="field-error show">{{ $message }}</p>@enderror
            </div>
            <!-- Strength meter -->
            ...
            <button type="submit" class="btn-terra w-full py-3.5 text-white font-semibold rounded-xl text-sm tracking-wide flex items-center justify-center gap-2">
