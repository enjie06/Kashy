...
tailwind.config = {
  theme: {
    extend: {
      ...
      boxShadow: { card: '0 2px 18px 0 rgba(60,40,10,.07)', btn: '0 4px 14px 0 rgba(196,154,108,.35)' }
    }
  }
}
</script>
...
.card { background:#fff; border:1px solid #E0D8CE; border-radius:16px; box-shadow:0 2px 18px 0 rgba(60,40,10,.07); padding:20px; }
...
<main id="main" class="min-h-screen bg-kashy-cream">
  ...
  <div class="px-5 md:px-8 py-6 max-w-2xl mx-auto">
    ...
    <!-- Skeleton loading -->
    <div id="formSkeleton" class="flex flex-col gap-5">
      <div class="skeleton h-16 w-full"></div>
      <div class="skeleton h-36 w-full"></div>
      <div class="skeleton h-48 w-full"></div>
    </div>

    <!-- Form (tersembunyi saat loading) -->
    <form id="storeForm" onsubmit="simpanProfil(event)" class="flex flex-col gap-5" style="display:none!important">

      <!-- Nama Toko -->
      <div class="card fade-up d2">
        <label class="form-label">Nama Toko</label>
        <input type="text" id="namaToko" class="form-input" placeholder="Nama toko Anda"/>
      </div>
      ...
      <div class="card fade-up d4">
        ...
        <div class="flex flex-col gap-3">
          <div>
            ...
            <input type="text" id="jalan" class="form-input"/>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="form-label">Kota</label>
              <input type="text" id="kota" class="form-input"/>
            </div>
            <div>
              <label class="form-label">Kode Pos</label>
              <input type="text" id="kodePos" class="form-input"/>
            </div>
          </div>
          <div>
...
<script>
  ...
  async function loadStore() {
    try {
      const res  = await fetch('{{ route("owner.store.show") }}');
      ...
      // Simpan untuk fungsi reset
      originalData = { ...data };

      // Sembunyikan skeleton, tampilkan form
      document.getElementById('formSkeleton').style.display = 'none';
      document.getElementById('storeForm').style.removeProperty('display');
    } catch (err) {
      document.getElementById('formSkeleton').innerHTML = `
        <div class="card text-center py-8">
          <p class="text-red-500 text-sm">Gagal memuat data toko. Coba refresh halaman.</p>
        </div>`;
    }
  }
  ...
  async function simpanProfil(e) {
    ...
    btn.disabled = true;
    btn.innerHTML = `<svg class="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Menyimpan...`;
    ...
    if (!payload.nama_toko) {
      showToast('Nama toko tidak boleh kosong.', false);
      btn.disabled = false;
      btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg> Simpan Profil`;
      return;
    }

    try {
      const res  = await fetch('{{ route("owner.store.update") }}', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        originalData = { ...payload, kode_pos: payload.kode_pos }; // update original
      } else {
  ...
  // ========= BUANG PERUBAHAN (reset ke data server) =========
  function buangPerubahan() {
    if (!confirm('Buang semua perubahan?')) return;
    document.getElementById('namaToko').value = originalData.nama_toko ?? '';
    document.getElementById('email').value    = originalData.email     ?? '';
    document.getElementById('phone').value    = originalData.phone     ?? '';
    document.getElementById('jalan').value    = originalData.jalan     ?? '';
    document.getElementById('kota').value     = originalData.kota      ?? '';
    document.getElementById('kodePos').value  = originalData.kode_pos  ?? '';
    document.getElementById('negara').value   = originalData.negara    ?? 'Indonesia';
