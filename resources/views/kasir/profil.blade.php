  ...
  <span class="w-1.5 h-1.5 rounded-full bg-terra"></span> Kasir
    ...
    <div class="anim-1 bg-white rounded-2xl shadow-sm border border-border overflow-hidden">
      ...
      <div class="info-row flex items-center gap-4 px-5 py-4 border-b border-stone-100">
        ...
        <div><p class="text-[10px] font-bold tracking-wide text-muted">Phone Number</p><p class="text-sm font-medium text-gray-900">{{ $user->phone ?? '-' }}</p></div>
      </div>
    ...
    <div class="anim-3 bg-white rounded-2xl shadow-sm border border-border overflow-hidden">
      <div class="px-5 pt-5 pb-6">
        ...
        <form action="{{ route('kasir.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
          @csrf
          @method('PUT')

          <div><label class="block text-[10px] font-bold tracking-wide text-muted mb-2">Foto Profil</label><input type="file" name="profile_photo" id="profile_photo" accept="image/*" class="kashy-input w-full px-4 py-3 border border-border rounded-xl bg-bg text-sm"></div>
    ...
    <div class="anim-4 bg-white rounded-2xl shadow-sm border border-border overflow-hidden">
      <div class="px-5 pt-5 pb-6">
        ...
        <form action="{{ route('kasir.password.update') }}" method="POST" class="space-y-4">
          @csrf
          @method('PUT')

          <div><label class="block text-[10px] font-bold tracking-wide text-muted mb-2">Password Lama</label><div class="relative"><input type="password" name="current_password" id="pwLama" class="kashy-input w-full px-4 py-3 pr-11 border border-border rounded-xl bg-bg text-sm"><button type="button" onclick="togglePw('pwLama','eyeL')" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-stone-300 hover:text-terra" id="eyeL"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg></button></div></div>
          <div><label class="block text-[10px] font-bold tracking-wide text-muted mb-2">Password Baru</label><div class="relative"><input type="password" name="password" id="pwBaru" class="kashy-input w-full px-4 py-3 pr-11 border border-border rounded-xl bg-bg text-sm"><button type="button" onclick="togglePw('pwBaru','eyeB')" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-stone-300 hover:text-terra" id="eyeB"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg></button></div></div>
          <div><label class="block text-[10px] font-bold tracking-wide text-muted mb-2">Konfirmasi Password</label><div class="relative"><input type="password" name="password_confirmation" id="pwKonfirm" class="kashy-input w-full px-4 py-3 pr-11 border border-border rounded-xl bg-bg text-sm"><button type="button" onclick="togglePw('pwKonfirm','eyeK')" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-stone-300 hover:text-terra" id="eyeK"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg></button></div></div>
