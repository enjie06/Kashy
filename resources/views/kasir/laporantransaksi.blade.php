...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .stat-sub { font-size:12px; color:#9C8B7E; margin-top:4px; }

  .section-header {
    font-size:14px; font-weight:700; color:#1C1C1C; margin-bottom:14px;
  }

  .cat-row {
    display:flex; align-items:center; gap:10px;
    padding:10px 0; border-bottom:1px solid #EAE0D6;
  }
  .cat-row:last-child { border-bottom:none; }
  .cat-name   { font-size:13px; font-weight:500; color:#1C1C1C; flex:1; }
  .cat-amount { font-size:13px; font-weight:700; color:#1C1C1C; white-space:nowrap; }
  ...
  .rank-table tbody td { padding:10px 0; font-size:13px; color:#1C1C1C; vertical-align:middle; }
  .rank-table tbody td:last-child { text-align:right; font-weight:700; }

  .rank-num { width:24px; height:24px; border-radius:50%; background:#F5F0EB; display:inline-flex; align-items:center; justify-content:center; font-size:11px; font-weight:700; color:#9C8B7E; flex-shrink:0; }
  ...
  .loading-spinner {
    ...
    border:2px solid #C8966C; border-radius:50%;
    border-top-color:transparent;
    animation:spin .6s linear infinite;
  }
  @keyframes spin { to { transform:rotate(360deg); } }
  .loading-state { display:flex; justify-content:center; align-items:center; padding:40px; gap:12px; }
</style>
</head>
...
<main class="flex-1 overflow-y-auto pb-28 max-w-2xl mx-auto w-full">
  ...
  <div class="px-4 flex flex-col gap-4 pb-4">
    ...
    <div id="loadingState" class="loading-state">
      ...
      <span class="text-muted text-sm">Memuat data laporan...</span>
    </div>

    <!-- Konten laporan -->
    <div id="laporanContent" class="flex flex-col gap-4" style="display:none;"></div>
...
<script>
  ...
  async function loadLaporan() {
    ...
    try {
      ...
      if (result.success) {
        ...
        laporanContent.innerHTML = `
          ...
          <div class="stat-card fade-up delay-3">
            ...
            ${data.penjualan_per_kategori.length > 0
              ? data.penjualan_per_kategori.map(kat => `
                  <div class="cat-row">
                    <div class="flex-1">
                      <div class="cat-name">${kat.name}</div>
                      <div class="bar-track mt-1.5"><div class="bar-fill" style="width:${kat.percentage}%"></div></div>
                    </div>
                    <span class="cat-amount">${formatRupiah(kat.total)}</span>
                  </div>`).join('')
              : '<p class="text-center text-muted text-sm py-4">Belum ada data penjualan</p>'}
          </div>
            ...
            <table class="rank-table">
              ...
              <tbody>
                ${data.produk_terlaris.length > 0
                  ? data.produk_terlaris.map(prod => `
                      <tr>
                        <td><span class="rank-num ${prod.rank <= 3 ? 'top' : ''}">${prod.rank}</span></td>
                        <td>${prod.name}</td>
                        <td style="text-align:right;">${prod.terjual}</td>
                      </tr>`).join('')
                  : '<tr><td colspan="3" class="text-center text-muted text-sm py-4">Belum ada data produk terjual</td></tr>'}
              </tbody>
            </table>
          </div>
          ...
          <div class="stat-card fade-up delay-5">
            ...
            ${data.metode_pembayaran.length > 0
              ? data.metode_pembayaran.map(met => `
                  <div class="cat-row">
                    <div class="flex-1">
                      <div class="cat-name">${met.name}</div>
                      <div class="bar-track mt-1.5"><div class="bar-fill" style="width:${met.percentage}%"></div></div>
                    </div>
                    <span class="cat-amount">${met.percentage}%</span>
                  </div>`).join('')
              : '<p class="text-center text-muted text-sm py-4">Belum ada data pembayaran</p>'}
          </div>
        `;

        loadingState.style.display  = 'none';
        laporanContent.style.display = 'flex';
      ...
      loadingState.innerHTML = '<p class="text-center text-red-500 text-sm py-4">Gagal memuat data laporan</p>';
    }
  }
